...
def test_render_route_output(fixture_graph, tmp_path):
    ...
    result = SimpleNamespace(
        ...
        rail_legs=[],
        bike_legs=[
            SimpleNamespace(url="https://maps.google/x", from_place="Start", to_place="End", time_label="~1:00 h")
        ],
        waypoints=[],
        composition=route_composition(track=track),
        gpx_path=gpx,
...
def test_rail_link_buttons():
    ...
    assert bahn_label == "🚉 Train 1: ~0:28 h" and bahn_url == leg.bahn_url


def test_bike_link_buttons():
    # One 🗺️ button per pedalled leg; label carries "Bike Route N: from → to" PLUS the time span; help fixed.
    leg = SimpleNamespace(url="https://maps.google/x", from_place="Freudenstadt", to_place="Horb", time_label="~0:30 h")
    ...
    assert label == "🗺️ Bike Route 1: Freudenstadt → Horb (~0:30 h)" and url == "https://maps.google/x"
