...
def _rail_graph() -> nx.MultiDiGraph:
    """S(bike) → station A → station B (rail, uphill) with transfer legs.
    ...
    graph.add_node(3, x=8.10, y=48.0, elevation=600.0, is_station=True)  # station B, higher
    # boarding transfer (enters station 2 → wait applied), then the uphill rail ride
...
def test_densify_track_follows_baked_3d_polyline_and_keeps_timing():
    ...
    assert max(p.elevation_m for p in dense.points) == 200.0  # baked apex elevation used
    assert dense.ascent_m == pytest.approx(100.0)  # 100→200 climb from baked vertices
