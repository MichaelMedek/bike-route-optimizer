...
from tests.conftest import make_choice_graph, make_cutthrough_graph, make_line_graph


def _params(**ov: float) -> RoutingParams:
    base = dict(
...
def test_cutthrough_taken_when_detour_is_long_and_boarding_free():
    ...
    graph = make_cutthrough_graph(params=_params(extra_km_per_boarding=0.0), detour_m=10_000.0)
...
def test_cutthrough_avoided_when_boarding_is_expensive():
    ...
    graph = make_cutthrough_graph(params=_params(extra_km_per_boarding=50.0), detour_m=10_000.0)
...
def test_station_edge_cost_never_below_length_keeps_heuristic_admissible():
    ...
    for boarding in (0.0, 50.0):
        graph = make_cutthrough_graph(params=_params(extra_km_per_boarding=boarding))
