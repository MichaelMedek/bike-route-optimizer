...
```bash
# one-time: install the osmium CLI (build-only; pre-clips each region's pbf natively)
brew install osmium-tool          # macOS  (Linux: apt-get install osmium-tool)
